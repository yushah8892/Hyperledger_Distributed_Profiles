...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"ifmprofilechannel\")"
  ...
  echo
  echo "	fabricNetwork.sh generate -c ifmprofilechannel"
  echo "	fabricNetwork up -c ifmprofilechannel -s couchdb"
  echo "        fabricNetwork up -c ifmprofilechannel -s couchdb -i 1.4.0"
  echo "	fabricNetwork up -l node"
  echo "	fabricNetwork down -c ifmprofilechannel"
  echo
...
function checkPrereqs() {
  ...
  for UNSUPPORTED_VERSION in $BLACKLISTED_VERSIONS; do
    ...
    if [ $? -eq 0 ]; then
      echo "ERROR! Local Fabric binary version of $LOCAL_VERSION does not match this newer version of ifmprofile-network and is unsupported. Either move to a later version of Fabric or checkout an earlier version of ifmprofile-network."
      exit 1
    fi

    echo "$DOCKER_IMAGE_VERSION" | grep -q "$UNSUPPORTED_VERSION"
    if [ $? -eq 0 ]; then
      echo "ERROR! Fabric Docker image version of $DOCKER_IMAGE_VERSION does not match this newer version of ifmprofile-network and is unsupported. Either move to a later version of Fabric or checkout an earlier version of ifmprofile-network."
      exit 1
    fi
  done
}
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v "$PWD":/tmp/ifmprofilechannel --rm hyperledger/fabric-tools:"$IMAGETAG" rm -Rf /tmp/ifmprofilechannel/ledgers-backup
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/usb.ifmprofile-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/USB_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
  cd crypto-config/peerOrganizations/jpmc.ifmprofile-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/JPMC_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
  cd crypto-config/peerOrganizations/boa.ifmprofile-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/BOA_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
...
function generateChannelArtifacts() {
  ...
  set -x
  configtxgen -profile OrdererGenesis -channelID ifm-sys-channel -outputBlock ./channel-artifacts/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ProfileChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID "$CHANNEL_NAME"
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for usbMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile ProfileChannel -outputAnchorPeersUpdate ./channel-artifacts/usbMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg usbMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for usb..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for jpmcMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile ProfileChannel -outputAnchorPeersUpdate ./channel-artifacts/jpmcMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg jpmcMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for jpmc..."
    exit 1
  fi
  echo
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for boaMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile ProfileChannel -outputAnchorPeersUpdate ./channel-artifacts/boaMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg boaMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for boa..."
    exit 1
  fi
  echo
...
CLI_DELAY=5
# channel name defaults to "ifmprofilechannel"
